...
func add_inspector_buttons() -> Array:
	var buttons = []
	buttons.push_back(
		{
			"name": "Import",
			"icon": preload("res://addons/blender_to_godot_pipeline/icon.svg"),
			"pressed": _on_import_button_pressed
		}
	)
	return buttons
...
func _cast_to_type(value, type: String):
	...
	if type in custom_hanlders.keys():
